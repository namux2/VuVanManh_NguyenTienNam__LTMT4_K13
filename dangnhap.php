...
if ($_POST) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM users WHERE email='$user_email' AND password='$user_password'");
    $row = mysqli_fetch_assoc($result);
    
    if ($row) {
        if ($row['role'] === 'admin') {
            $_SESSION['login'] = $row['role'];
            echo "<script>alert('Bạn đã đăng nhập thành công với vai trò Admin.'); window.location.href='/VuVanManh_NguyenTienNam__LTMT4_K13/admin/advance-admin/index.php';</script>";
            exit;
        } else {
            $_SESSION['login'] = $row['role'];
        ...
        echo "<script>alert('Sai tên Email hoặc mật khẩu. Vui lòng thử lại.')</script>";
    }
}
?>
...
<body>
    ...
    <div class="container">
        <ul class="menu">
            ...
            <li><a href="dangnhap.html">Đăng Nhập <i class="gg-chevron-right"></i></a></li>
        </ul>
    </div>
    <div class="container">
        <div class="row">
    ...
    <div class="form-floating mb-3">
        <input type="email" class="form-control" id="user_email" name="user_name" placeholder="name@example.com" required>
        <label for="user_email">Email</label>
    </div>
    <div class="form-floating mb-3">
        <input type="password" class="form-control" id="user_password" name="user_pass" placeholder="Password" required>
    ...
    <div class="d-grid">
        <button class="btn btn-primary btn-login text-uppercase fw-bold" type="submit">Đăng Nhập</button>
                        ...
                        <div class="text-center mt-3">
                            <span>Chưa có tài khoản? </span><a href="dangky.php">Đăng ký</a>
                        </div>
                    </div>
                </div>
            </div>
